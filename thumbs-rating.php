...
if  ( ! function_exists( 'thumbs_rating_getlink' ) ):
	...
	function thumbs_rating_getlink($post_ID = '', $type_of_vote = '')
	{
		...
		$thumbs_rating_link = "";
		$icon_svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 206.6"><path d="M177 7l136 136c9.4 9.4 9.4 24.6 0 33.9l-22.6 22.6c-9.4 9.4-24.6 9.4-33.9 0L160 103.2l-96.4 96.4c-9.4 9.4-24.6 9.4-33.9 0L7 177c-9.4-9.4-9.4-24.6 0-33.9L143 7c9.4-9.3 24.6-9.3 34 0z"/></svg>';
		...
		$link_up = '<span class="rate__control rate__control--up padding--horizontal-tiny'. ( (isset($thumbs_rating_up_count) && intval($thumbs_rating_up_count) > 0 ) ? ' rate__control--voted' : '' ) .'" onclick="thumbs_rating_vote(' . $post_ID . ', 1);" data-text="' . __('Vote Up','thumbs-rating') . ' +" aria-label="click to vote positively on the game example and improve the examples overall ranking score.">' .$icon_svg . '</span>';
		$link_down = '<span class="rate__control rate__control--down padding--horizontal-tiny'. ( (isset($thumbs_rating_down_count) && intval($thumbs_rating_down_count) > 0 ) ? ' rate__control--voted' : '' ) .'" onclick="thumbs_rating_vote(' . $post_ID . ', 2);" data-text="' . __('Vote Down','thumbs-rating') . ' -" aria-label="click to vote negatively on the game example and lower the examples overall ranking score.">' .$icon_svg . '</span>';

		$thumbs_rating_total_count = $thumbs_rating_up_count - $thumbs_rating_down_count;

		$thumbs_rating_link = '<div class="rate padding--vertical-tiny flex flex--direction-column" id="thumbs-rating-'.$post_ID.'" data-content-id="'.$post_ID.'">';
		$thumbs_rating_link .= $link_up;
		$thumbs_rating_link .= '<span class="rate--total padding--tiny">' . $thumbs_rating_total_count . '</span>';
